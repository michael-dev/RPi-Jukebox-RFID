...
def bt_led(led_pin, isEnabled):
    if led_pin is None:
        return
    print(f"set LED {led_pin} to {isEnabled}")
    proc = subprocess.run(["/usr/bin/pinctrl", "set", led_pin, "op", ( "dh" if isEnabled else "dl")], shell=False,
...
def bt_find_led_pin(led_pin):
    # detect GPIO
    print(f"searching for led_pin={led_pin}")
    proc = subprocess.run(["/usr/bin/pinctrl", led_pin], 
